...
```bash
# 方式一：一键部署（自动探测硬件+推荐模型+拉取）
python scripts/setup_model.py

# 方式二：手动指定模型
python scripts/setup_model.py qwen3:7b
...
无需额外配置即可使用。在 Chrome 浏览器中：
1. 打开 `http://localhost:8000`
2. 点击命令栏右侧的 🎤 按钮
3. 按住说话，松开发送
4. 系统自动识别为设备指令 → 快速通道执行（<200ms）

要启用完全离线的语音转文字（Whisper STT）：
```bash
pip install openai-whisper
# 重启后端即可
# 前端会自动改用 MediaRecorder 发送音频到 /api/stt
```
...
支持通过自然语言控制设备。系统分三层处理优先级：

1. **EXACT_COMMANDS 精确匹配**（25条高频指令，O(1)查找，< 200ms）
2. **PATTERNS 正则匹配**（16条灵活模式，如 "调到XX度"）
3. **LLM 标准通道**（Ollama 本地模型解析模糊意图，需拉起模型）

```
"打开客厅灯"     →  精确匹配  → 灯亮了  ✅ (< 200ms)
"所有灯打开"     →  正则匹配  → 全亮    ✅ (< 200ms)
"空调调到26度"   →  正则匹配  → 26°C    ✅
"温馨一点"       →  LLM 通道  → 场景切换 🔄 (需 Ollama)
```

### 🎤 语音控制

无需额外配置即可在 Chrome 中使用：

- **Web Speech API**：按住 🎤 说话，松开发送（浏览器原生，零成本）
- **Whisper 离线 STT**：安装 `openai-whisper` 后自动切换（隐私优先）
- 语音 → 文字 → EXACT_COMMANDS → 执行，全程 < 500ms
...
┌──────────────────────────▼──────────────────────────────────┐
│                  ⚙️ 执行层                                     │
│  安全层 · IntentGate · 脱敏日志 · MQTT · 5个虚拟设备          │
└──────────────────────┬───────────────────────────────────────┘
                       │
┌──────────────────────▼───────────────────────────────────────┐
│            🗄️ 基础设施                                          │
│        MQTT Broker · SQLite · Ollama + Qwen3 · Crontab        │
└──────────────────────────────────────────────────────────────┘

数据流:
🎤 语音 → (STT) → 文字 → EXACT_COMMANDS(25条) → 命中 → 直接执行(<200ms)
                           → 未命中 → Sanitizer脱敏 → LLM解析 → 执行
                                                         → 提醒 → 存入crontab
```
...
| **前端** | Tailwind CSS + Web Speech API → Whisper | 渐进式增强 |
| **语音** | Web Speech API (Chrome) → Whisper (本地) | 按住说话，松开发送 |
| **测试** | pytest + pytest-cov + GitHub Actions | 94% 覆盖率, CI 门禁 80% |
...
│   ├── app/
│   │   ├── api/                 # REST API 路由
│   │   │   ├── devices.py       # 设备控制（含快慢路径分离）
│   │   │   ├── stt.py           # 语音转文字 (Whisper)
│   │   │   ├── reminders.py     # 提醒管理 CRUD
│   │   │   ├── health.py        # 系统健康 + 舒适度检测
│   │   │   └── skills.py        # Skill API
│   │   ├── core/                # 核心配置 & 数据库
│   │   ├── execution/           # 执行层
│   │   │   ├── sanitizer.py     # 敏感信息脱敏（正则+NER）
│   │   │   ├── comfort.py       # 舒适度规则引擎
│   │   │   ├── reminder.py      # 提醒系统（LLM→crontab）
│   │   │   └── safety.py        # 安全层
│   │   ├── llm/                 # LLM 推理引擎
│   │   │   ├── express.py       # 快速通道 (EXACT 25条+PATTERNS 16条)
│   │   │   ├── standard.py      # 标准通道 (Ollama LocalLLM)
│   │   │   ├── router.py        # 三级通道路由器
│   │   │   └── provisioner.py   # 硬件探测+模型推荐
│   │   ├── skill/               # Skill 管理系统 (11模块, 247测试)
│   │   ├── memory/              # 记忆系统（暂未实装）
│   │   ├── user/                # 多用户管理
│   │   └── main.py              # 应用入口
│   ├── tests/                   # 测试套件 (247 测试, 94% 覆盖率)
│   ├── requirements.txt
│   └── pyproject.toml
├── frontend/                    # 前端界面
│   └── index.html               # 仪表盘（设备面板+舒适度+提醒+语音）
├── MODELS.md                    # 推荐模型清单
├── scripts/
│   └── setup_model.py           # 一键模型部署脚本
├── skills/                      # Skill 仓库
│   ├── built-in/                # 内置 Skill
│   └── user-installed/          # 用户安装
├── docs/                        # 文档
├── .github/workflows/test.yml   # CI (覆盖率门禁 80%)
```
...
Phase 2 (第 4-6 周):   🐌 学会技能      ✅ 已完成
  Skill 系统 — 运行时/仓库/冲突仲裁/回滚沙箱/健康监测 (247 测试, 94% 覆盖率)

Phase 3 (第 7-10 周):  🐌 长出大脑      ✅ 已完成
  语音入口(STT) + 命令映射器(EXACT_COMMANDS 25条) + Sanitizer脱敏
  + 快慢路径分离 + 舒适度引擎(5规则) + 提醒系统(crontab) + 前端面板

Phase 4 (第 11-13 周): 🐌 认识家人      ⌛ 待开始
  多用户管理 + 设备自动发现 + LLM 角色定制(连接 Ollama 后)
